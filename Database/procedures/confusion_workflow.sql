-- ============================================================================
-- CONFUSION WORKFLOW PROCEDURES (V1)
-- Signal -> Case -> Assignment -> Official Answer -> Resolution
-- ============================================================================
...
BEGIN
  ...
  IF v_case_id IS NULL THEN
    INSERT INTO public.resource_confusion_cases(
      resource_id,
      module_id,
      student_id,
      status,
    ...
    VALUES (
      ...
      p_module_id,
      p_user_id,
      'nouveau'::public.confusion_case_status,
      NOW(),
      NOW()
    )
  ...
  IF v_assignee IS NULL THEN
    ...
    IF v_assignee IS NOT NULL THEN
      ...
      SET assigned_to_user_id = v_assignee,
          ...
          status = CASE
            WHEN status = 'nouveau'::public.confusion_case_status THEN 'assigne'::public.confusion_case_status
            ELSE status
          END,
          updated_at = NOW()
      WHERE id = v_case_id;
      ...
      VALUES (
        ...
        'auto_assigned',
        NULL,
        jsonb_build_object('assigned_to_user_id', v_assignee, 'source', v_source)
      );
...
CREATE OR REPLACE FUNCTION public.sp_confusion_case_get_by_id(
  p_case_id BIGINT
)
RETURNS TABLE(
  id BIGINT,
...
BEGIN
  ...
  SET assigned_to_user_id = p_assignee_user_id,
      ...
      status = CASE
        WHEN c.status = 'nouveau'::public.confusion_case_status THEN 'assigne'::public.confusion_case_status
        ELSE c.status
      END,
      updated_at = NOW()
  WHERE c.id = p_case_id
  ...
  IF FOUND THEN
    ...
    VALUES (
      ...
      jsonb_build_object('assigned_to_user_id', p_assignee_user_id, 'reason', NULLIF(TRIM(p_reason), ''))
    );
  END IF;
END;
$$;
...
BEGIN
  ...
  SET official_answer_id = p_answer_id,
      status = CASE
        WHEN c.status = 'resolu'::public.confusion_case_status THEN c.status
        ELSE 'repondu_officiel'::public.confusion_case_status
      END,
      updated_at = NOW()
  ...
  IF FOUND THEN
    ...
    SELECT
      ...
      p_actor_user_id,
      jsonb_build_object('question_id', p_question_id, 'answer_id', p_answer_id)
    FROM public.resource_confusion_cases c
    WHERE c.id IN (
      SELECT c2.id
