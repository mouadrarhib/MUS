...
router.post(
  ...
  requireRole("teacher", "student", "admin"),
  [
    body("available_date").isDate().withMessage("available_date must be a valid date YYYY-MM-DD"),
    body("available_time").matches(/^\d{2}:\d{2}(:\d{2})?$/).withMessage("available_time must be in HH:MM format"),
    body("duration_minutes").isInt({ min: 1 }).withMessage("duration_minutes must be > 0"),
    body("price").isFloat({ min: 0 }).withMessage("price must be a positive decimal number"),
...
router.patch(
  ...
  [
    ...
    body("available_date").optional().isDate().withMessage("available_date must be a valid date YYYY-MM-DD"),
    body("available_time").optional().matches(/^\d{2}:\d{2}(:\d{2})?$/).withMessage("available_time must be in HH:MM format"),
    body("duration_minutes").optional().isInt({ min: 1 }).withMessage("duration_minutes must be > 0"),
    body("price").optional().isFloat({ min: 0 }).withMessage("price must be a positive decimal number"),
    body("timezone").optional().isString().isLength({ min: 2, max: 64 }).withMessage("timezone must be a valid string"),
    body("is_active").optional().isBoolean().withMessage("is_active must be boolean"),
  ],
  validateRequest,
